...
$dataRuteDetail = "SELECT rute_details.*, checkpoint.nama_checkpoint, checkpoint.id_checkpoint FROM rute_details JOIN checkpoint ON ".
        ...
        "AND rute_details.id_user = '".$_SESSION['id']."' LIMIT 1";
$rDataRuteDetail = mysqli_query($link,$dataRuteDetail) or die(mysqli_error($link));
$hDataRuteDetail = mysqli_fetch_assoc($rDataRuteDetail);

if ($hDataRuteDetail['scanned'] == 1) {
  echo "<script>
...
// apabila zona sebelumnya belum terselesaikan
    ...
    ";
        ...
        if (mysqli_query($link, $query3)) {
            ...
            if (mysqli_query($link, $updateRuteDetail)) {
                $checkJumlah = "SELECT * FROM rute_details WHERE user_id = '".$_SESSION['id']."' AND scanned = '0'";
                $rCheckJumlah = mysqli_query($link, $checkJumlah);
                $hCheckJumlah = mysqli_num_rows($rCheckJumlah);
